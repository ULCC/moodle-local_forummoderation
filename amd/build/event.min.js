...
/**
   ...
   * */Object.defineProperty(_exports,"__esModule",{value:!0}),_exports.default=void 0,_jquery=_interopRequireDefault(_jquery),_selectors=_interopRequireDefault(_selectors),_services=_interopRequireDefault(_services),_customformdata=_interopRequireDefault(_customformdata),_helpers=_interopRequireDefault(_helpers);var _default=class{constructor(){this.reportPostSelector=_selectors.default.actions.reportPost,this.eventListeners={}}init(){(new _helpers.default).createLinkReportModerator()}addEvent(eventType,handler){this.eventListeners[eventType]||(this.eventListeners[eventType]=[],document.addEventListener(eventType,(evt=>{this.eventListeners[eventType].forEach((listener=>{evt.target.closest(listener.selector)&&listener.handler(evt)}))}))),this.eventListeners[eventType].push({selector:this.reportPostSelector,handler:handler})}reportPostForum(evt){evt.preventDefault();const spinner=(new _helpers.default).createSpinner("spinner"),parentElement=evt.target;parentElement.insertAdjacentElement("afterend",spinner),(0,_jquery.default)(evt.target).hide(),(0,_jquery.default)(spinner).show();const postId=parentElement.getAttribute("data-post-id"),formData=this.prepareFormDataReport(postId,"");new _services.default(_jquery.default.ajax).reportPost(formData,(res=>{const{success:success}=JSON.parse(res);success&&window.location.reload()}))}prepareFormDataReport(postId,message){return new _customformdata.default({postId:parseInt(postId),courseId:M.cfg.courseId,contextId:M.cfg.contextid,courseContextId:M.cfg.courseContextId,contextInstanceId:M.cfg.contextInstanceId,action:"report-post",message:message,sesskey:M.cfg.sesskey}).toFormData()}};return _exports.default=_default,_exports.default}));
